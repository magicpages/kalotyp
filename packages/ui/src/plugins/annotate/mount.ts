...
export function mountAnnotateUtility(options: MountAnnotateOptions): MountAnnotateHandle {
  ...
  /** The selected shape to draw handles for, or `null` while the text editor is open. */
  ...
  // ----- Inline text editor -----
  // True while the inline editor is open. The selection handles are suppressed
  // during editing so the user sees one box (the editor), not the editor plus
  // the handle frame stacked on top.
  ...
  /**
   * Open the inline editor on a text shape and suppress the selection handles
   * while editing, so only the editor box shows (not the handle frame too).
   */
  ...
  // Load the annotation web fonts (idempotent; Ghost may have them already).
