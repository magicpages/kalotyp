...
The three published packages (`@magicpages/kalotyp-core`, `@magicpages/kalotyp-ui`, `@magicpages/kalotyp-ghost`) are linked, so they release together with a single version. The two `apps/*` workspaces are ignored — they're internal harnesses, not published.