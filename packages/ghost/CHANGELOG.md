...
- 1210270: Fix the editor leaving Ghost's React admin stuck after a save. `openDefaultEditor()` now returns an instance with a `destroy()` method, and the editor emits a `destroy` event whenever it tears itself down.
