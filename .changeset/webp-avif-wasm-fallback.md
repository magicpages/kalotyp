...
A transient failure loading the WASM codec (network blip, jsDelivr hiccup) no longer permanently breaks WebP/AVIF export for the rest of the session — the next attempt retries a fresh fetch instead of reusing a cached failure.

`'auto'` format selection, which now always prefers WebP, retries with a natively-supported format (JPEG or PNG) if the WASM encode fails, instead of failing the save outright. An explicit WebP/AVIF choice still surfaces the failure rather than silently substituting a different format.
